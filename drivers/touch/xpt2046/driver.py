...
class Touch(object):
    ...
    def __init__(self, int_pin=None, int_handler=None,
        ...
        """
        self.spi = SoftSPI(sck=Pin(SpiConf.clk), mosi=Pin(SpiConf.mosi), miso=Pin(SpiConf.miso))
    ...
    def raw_touch(self):
        ...
        Returns:
            tuple(int, int): X, Y
        """
        #inverted because display is upsidedown
    ...
    def single_touch(self):
        touch_data = self.raw_touch()
        
        if self.touch_down and touch_data == None:
            if self.count_threshold == self.touch_up_threshold:
                self.touch_down = False
                self.count_threshold = 0
            
            self.count_threshold += 1
            return None
        elif not self.touch_down and touch_data != None:
            self.touch_down = True
            return self.normalize(touch_data[0], touch_data[1])
        elif touch_data != None and self.count_threshold != 0:
            self.count_threshold = 0
